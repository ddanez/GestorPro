...
import { CompanyInfo, AppSettings, Sale } from '../types';
...
export const TicketModal: React.FC<Props> = ({ isOpen, onClose, data, company, settings, title = "Comprobante" }) => {
  ...
  // Cálculos precisos para el desglose del ticket
  const itemsSubtotal = (data.items || []).reduce((acc: number, item: any) => acc + (item.quantity * item.priceUSD), 0);
  const discount = data.discountUSD || 0;
  const totalVenta = itemsSubtotal - discount;
  const pagos = data.paidAmountUSD || 0;
  const deuda = totalVenta - pagos;
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.download = `Ticket_${(data.customerName || 'Venta').replace(/\s+/g, '_')}_${data.id.slice(0,6).toUpperCase()}.png`;
      link.href = dataUrl;
      link.click();
      ...
      try {
        ...
        const link = document.createElement('a');
        link.download = `Ticket_${data.id.slice(0,6).toUpperCase()}.png`;
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-[500] flex items-center justify-center p-2 backdrop-blur-sm overflow-y-auto print:p-0 print:bg-white">
      <div className="w-full max-w-sm bg-white rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 print:shadow-none print:rounded-none">
        ...
        {/* El Ticket POS (Contenedor de captura) */}
        <div 
          ref={ticketRef} 
        ...
        >
           ...
           <div className="text-center space-y-0.5 mb-4">
              ...
              <div className="text-[9px] font-bold space-y-0.5 mt-1 border-t border-slate-100 pt-1">
                <p>RIF: {company.rif} {company.dni && `| TITULAR: ${company.dni}`}</p>
           ...
           <div className="space-y-1 mb-4">
              <div className="flex items-baseline justify-between">
                 <span className="font-bold uppercase text-[9px]">CLIENTE:</span>
                 <span className="uppercase font-black text-[13px] text-right">{data.customerName || 'CONTADO'}</span>
              </div>
              ...
              <div className="border-t border-black pt-2 mt-2 space-y-1">
                 ...
                 <div className="flex justify-between items-center whitespace-nowrap overflow-hidden">
                    ...
                    <span className="font-bold text-[10px] text-right truncate ml-2">
                      {new Date(data.date).toLocaleDateString('es-VE')} {new Date(data.date).toLocaleTimeString('es-VE', { hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true })}
                    </span>
                 </div>
                 <div className="flex justify-between items-center">
                    ...
                    <span className="font-black uppercase bg-black text-white px-2 py-0.5 text-[9px]">
                      {data.status === 'paid' ? 'PAGADO' : 'A CRÉDITO'}
                    </span>
                 </div>
              </div>
           </div>
           ...
           <div className="mb-4">
              <div className="flex justify-between font-black text-[9px] uppercase border-b border-black pb-1 mb-1">
                 ...
                 <span className="w-1/2 text-right">SUBTOTAL</span>
              </div>
                 ...
                 {(data.items || []).map((item: any, i: number) => (
                   <div key={i} className="py-2">
                      ...
                      <div className="flex justify-between text-[12px] items-baseline">
                         <span className="w-1/4 font-bold text-left">{item.quantity.toFixed(2).replace('.', ',')}</span>
                         <span className="w-1/4 text-center">{item.priceUSD.toFixed(2).replace('.', ',')}</span>
                         <span className="w-1/2 text-right font-black">{(item.quantity * item.priceUSD).toFixed(2).replace('.', ',')}</span>
                      </div>
                   </div>
                 ))}
              </div>
           ...
           <div className="border-t border-black pt-3 space-y-1 mb-5">
              <div className="flex justify-between text-[12px]">
                 ...
                 <span className="font-black">US${itemsSubtotal.toFixed(2).replace('.', ',')}</span>
              </div>
              
              {discount > 0 && (
                <div className="flex justify-between text-[12px]">
                   <span className="font-bold uppercase">DESCUENTO</span>
                   <span className="font-black">- US${discount.toFixed(2).replace('.', ',')}</span>
                </div>
              )}

              <div className="flex justify-between text-[12px] border-t border-slate-300 pt-1">
                 <span className="font-bold uppercase">TOTAL VENTA</span>
                 <span className="font-black">US${totalVenta.toFixed(2).replace('.', ',')}</span>
              </div>

              <div className="flex justify-between text-[12px]">
                 <span className="font-bold uppercase">PAGOS / ABONOS</span>
                 <span className="font-black">- US${pagos.toFixed(2).replace('.', ',')}</span>
              </div>

              <div className="flex justify-between text-[16px] font-black border-t-2 border-double border-black pt-2 mt-1">
                 <span className="uppercase">SALDO DEUDA</span>
                 <span className="text-right">US${deuda.toFixed(2).replace('.', ',')}</span>
              </div>

              {settings.exchangeRate > 0 && (
                <div className="text-right text-[10px] italic mt-1 font-bold border-t border-dotted border-slate-300 pt-1">
                  Ref en BS: {(deuda * settings.exchangeRate).toLocaleString('es-VE', { minimumFractionDigits: 2 })} Bs
                </div>
              )}
           </div>
           ...
           <div className="border-t border-black pt-5 mb-5">
              <div className="border border-dotted border-black p-3 text-center bg-slate-50">
                 <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-2 leading-none">Información de Pago</p>
                 <div className="space-y-1">
                    <p className="uppercase font-black text-[12px] leading-none">{company.bank || 'BANCO DE VENEZUELA'}</p>
                    <p className="font-black text-[14px] leading-none">{company.mobilePhone || '0412 959 3598'}</p>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-col gap-3 print:hidden">
           ...
           >
              {isGenerating ? 'Generando Imagen...' : <><Download size={20} /> Descargar Ticket (Imagen)</>}
           </button>
           
           <div className="flex gap-3">
              <button 
                onClick={handlePrint} 
                className="flex-1 bg-slate-800 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2"
              >
                 <Printer size={16} /> Imprimir
              </button>
              <button 
                onClick={onClose} 
                className="flex-1 bg-slate-200 text-slate-700 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-300 transition-colors"
              >
                 Salir
              </button>
           </div>
