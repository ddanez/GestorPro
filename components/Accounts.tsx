...
const Accounts: React.FC<Props> = ({ type, items, settings, company, onUpdate }) => {
  ...
  const handleProcessPayment = async () => {
    ...
    setTicketData({ ...updatedItem, totalUSD: amountToPay, totalBS: amountToPay * settings.exchangeRate, date: new Date().toISOString() });
    setPaymentModal(null);
    setAmountToPay(0);
    onUpdate();
  };

  const totalPending = items.reduce((sum, item) => sum + (item.totalUSD - (item.paidAmountUSD || 0)), 0);
    ...
    <div className="space-y-4 animate-in fade-in duration-500">
      <div className="bg-[#1e293b] p-4 rounded-2xl border border-slate-700 shadow-lg flex justify-between items-center">
         <div>
            ...
            <p className="text-2xl font-black text-white leading-none">${totalPending.toFixed(2)}</p>
         </div>
      ...
      <div className="space-y-3">
        {items.map(item => {
          const balance = item.totalUSD - (item.paidAmountUSD || 0);
          return (
            <div key={item.id} className="bg-[#1e293b] p-4 rounded-2xl border border-slate-700 flex justify-between items-center gap-4 hover:border-slate-500 transition-colors">
               <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="truncate">
                     ...
                     <p className="text-[8px] text-rose-500 font-black uppercase mt-0.5">Pendiente: ${balance.toFixed(2)}</p>
                  </div>
               </div>
               <button 
